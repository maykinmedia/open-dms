...
* ``DB_NAME``: name of the PostgreSQL database. Defaults to: ``opendms``.
* ``DB_USER``: username of the database user. Defaults to: ``opendms``.
* ``DB_PASSWORD``: password of the database user. Defaults to: ``opendms``.
...
* ``ELASTIC_APM_SERVICE_NAME``: Name of the service for this application in Elastic APM. Defaults to ``opendms - <environment>``.
...
* ``DEFAULT_FROM_EMAIL``: The default email address from which emails are sent. Defaults to: ``opendms@example.com``.
